...
MultiGraph = collections.namedtuple('Graph', ['node_features', 'reciever_features', 'sender_features', 'adj', 'edge_sets'])
...
class GraphNetBlock(nn.Module):
    ...
    def _update_edge_features(self, node_features, reciever_features, sender_features):
        ...
        """
        next_reciever_features = self.reciever_model(torch.cat([node_features, reciever_features], dim=1))
        next_sender_features = self.sender_model(torch.cat([node_features, sender_features], dim=1))
    ...
    def _update_node_features(self, node_features, reciever_features, sender_features, adj):
        ...
        reciever_features_key = self.key_linear(reciever_features).view(num_nodes, self.num_heads, self.hidden_size)
        sender_features_key = self.key_linear(sender_features).view(num_nodes, self.num_heads, self.hidden_size)

        qr = torch.einsum('ijk,ijk->ji', query_features_head, reciever_features_key)
        qr = qr.view(self.num_heads, num_nodes, 1).repeat(1, 1, num_nodes)
        qs = torch.einsum('ijk,ljk->jil', query_features_head, sender_features_key)
        ...
        sender_features_value = self.key_linear(sender_features).view(num_nodes, self.num_heads, self.hidden_size)
        reciever_features_value = self.key_linear(reciever_features)

        aggr_sender_head = torch.einsum('ijk,kil->jil', attn_score, sender_features_value)
        next_node_features = reciever_features_value + aggr_sender_head.contiguous().view(num_nodes, self.num_heads*self.hidden_size)

        next_node_features = self.node_linear(next_node_features)
    ...
    def forward(self, graph):
        ...
        return MultiGraph(new_node_features, new_reciever_features, new_sender_features, graph.adj, None)
...
class Encoder(nn.Module):
    ...
    def forward(self, graph):
        ...
        features_r = [node_latents]
        features_s = [node_latents]
        for edge_set in graph.edge_sets:
            features_r.append(
                self.unsorted_segment_operation(edge_set.features, edge_set.recievers, num_nodes))
            features_s.append(
                self.unsorted_segment_operation(edge_set.features, edge_set.senders, num_nodes))
        features_r = torch.cat(features_r, dim=-1)
        features_s = torch.cat(features_s, dim=-1)

        reciever_latents = self.reciever_model(features_r)
        sender_latents = self.sender_model(features_s)

        return MultiGraph(node_latents, reciever_latents, sender_latents, graph.adj, None)
