...
#input with node_type and velocity
...
from mesh_tie import core_model_1
...
class Model(nn.Module):
    ...
    def __init__(self, params, learned_model):
        ...
        self._reciever_normalizer = normalization.Normalizer(size=3 + common.NodeType.SIZE, name='reciever_normalizer')
        self._sender_normalizer = normalization.Normalizer(size=3 + common.NodeType.SIZE, name='sender_normalizer')
    ...
    def _build_graph(self, inputs, is_training):
        ...
        world_edges = core_model_1.EdgeSet(
            name='world_edges',
        ...
        world_connection_matrix[senders, recievers] = torch.tensor(True, dtype=torch.bool, device=device)
        adj = world_connection_matrix
        ...
        """
        """
        reciever_features = torch.cat([inputs['world_pos'], inputs['mesh_pos']], axis=-1)
        sender_features = torch.cat([inputs['world_pos'], inputs['mesh_pos']], axis=-1)
        """
        
        """
        mesh_edges = core_model_1.EdgeSet(
            name='mesh_edges',
        ...
        return (core_model_1.MultiGraph(node_features=self._node_normalizer(node_features),
                                        reciever_features=self._reciever_normalizer(node_features),
                                        sender_features=self._sender_normalizer(node_features),
                                        adj=adj))
    ...
    def forward(self, inputs, is_training):
        ...
        if is_training:
            return self._learned_model(graph,
                                      world_edge_normalizer=None, is_training=is_training)
        else:
            return self._update(inputs, self._learned_model(graph,
                                                           world_edge_normalizer=None,
    ...
    def save_model(self, path, epoch=None):
        if epoch == None: 
            ...
            torch.save(self._reciever_normalizer, path + "/_reciever_normalizer"+str(epoch)+".pth")
            torch.save(self._sender_normalizer, path + "/_sender_normalizer"+str(epoch)+".pth")
    ...
    def load_model(self, path, epoch=None):
        if epoch == None: 
            ...
            self._reciever_normalizer = torch.load(path + "/_reciever_normalizer"+str(epoch)+".pth")
            self._sender_normalizer = torch.load(path + "/_sender_normalizer"+str(epoch)+".pth")
