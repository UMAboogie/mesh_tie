...
def _rollout(model, initial_state, num_steps):
    ...
    def step_fn(velocity, density, trajectory):
        with torch.no_grad():
            prediction1, prediction2 = model({**initial_state,
                                'velocity': velocity}, is_training=False)
            # don't update boundary nodes
        next_velocity = torch.where(mask, torch.squeeze(prediction1), torch.squeeze(velocity))
        next_density = torch.where(mask, torch.squeeze(prediction2), torch.squeeze(density))
...
def evaluate(model, trajectory):
    ...
    scalars = {'mse_%d_steps' % horizon: torch.mean(error[horizon])
                for horizon in [1, 10, 20, 50, 100, 200, -1]}
