...
class Model(nn.Module):
    ...
    def __init__(self, params, learned_model):
        ...
        self._mesh_edge_normalizer = normalization.Normalizer(
            size=8, name='mesh_edge_normalizer')  # 3D coord + 3D coord + 2*length = 8
    ...
    def _build_graph(self, inputs, is_training):
        ...
        no_connection_mask = torch.eq(node_type[:, 0], torch.tensor([common.NodeType.OBSTACLE.value], device=device))
        no_connection_mask_t = torch.stack([no_connection_mask] * world_pos.shape[0], dim=1)
        ...
        connection_mask = torch.transpose(torch.stack([no_connection_mask] * world_pos.shape[0], dim=1), 0, 1)
        ...
        world_recievers, world_senders = torch.nonzero(world_connection_matrix, as_tuple=True)
        ...
        world_edges = core_model_2.EdgeSet(
            name='world_edges',
            features=self._world_edge_normalizer(world_edge_features, is_training),
            recievers=world_recievers,
            senders=world_senders)
        
        world_connection_matrix[recievers, senders] = torch.tensor(True, dtype=torch.bool, device=device)
        ...
        return (core_model_2.MultiGraph(node_features=node_features,
                                        reciever_features=None, sender_features=None,
